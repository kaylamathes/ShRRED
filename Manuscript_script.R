...
FR_total_summary <- FR_total%>%
  ...
  summarize(ave_SA_V = mean(SA_V), ave_root_mass_g = mean(Dry_root_adjusted_g), ave_root_length = mean(Length.cm.), ave_root_volume = mean(RootVolume.cm3.), ave_root_surf_area = mean(SurfArea.cm2.), ave_root_diam = mean(AvgDiam.mm.), ave_biomass = mean(biomass_kg_m3))%>%
  mutate(Plot_ID = case_when(Type == "Treatment" & Plot == "1" ~ "T1", 
                             Type == "Treatment" & Plot == "2" ~ "T2",
                             Type == "Treatment" & Plot == "3" ~ "T3",
                             Type == "Treatment" & Plot == "4" ~ "T4",
...
##Exudation ANOVA
##Equality of variance test for Type:
leveneTest(NPOC_mg_g ~ Type, data = Exudates_summary)


Exudate_model <- aov(NPOC_mg_g ~ Type, data = Exudates_summary)
summary(Exudate_model)
...
########Roots NSC (girdled/control): Figure 4 #########
...
Plot_Nh4 <- ggplot(rhizosphere_dataframe_N , aes(x = Type2, y = ave_ammonium, fill = Type)) + 
  ...
  theme_classic() +
  scale_fill_manual(values = c("#8F6B2B","#4967D7")) +
  theme(axis.title = element_text(size = 40), axis.text = element_text(size = 35), panel.border = element_rect(colour = "black", fill = NA, size = 0.9), legend.position = "none") +
  labs(x = "Plot Type",y=expression(paste(" ",Soil," ",NH[4]^"+"," ",(mg~NH[4]^"+"/kg~soil),""))) +
...
ggsave(path = "Figures", filename = "ammonium.png", height = 15, width = 15, units = "in")
...
################################ nitrogen and nongirdled NSC (Figure 7)  #############################
ggplot(nitrogen_NSC, aes(x = ave_ammonium, y = NSC_nongirdle, color = Type)) +
  geom_point(size = 4)+
  geom_smooth(method = "lm", se = FALSE, size = 2)+
  theme_classic() +
  scale_color_manual(values = c("#8F6B2B","#4967D7"), labels = c("Control Focal Trees", "Surviving focal trees \n in disturbance")) +
